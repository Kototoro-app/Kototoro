...
### Mistake 5: Recomputing a Unique Identity After Moving Ownership

When a repair operation moves a projection from one entity to another, a
deterministic identity derived from that projection may still be stored on
the old entity. Recomputing it for the new entity and writing it without an
ownership lookup causes a database unique-constraint failure.

Before updating a unique identity field across a repair boundary:

- Query the current owner of the derived value.
- Treat ownership by another entity as a conflict, not as permission to
  overwrite.
- Preserve the destination's existing unique identity or allocate a fresh
  fallback identity.
- Add a regression fixture that performs the move while the old owner still
  exists.
