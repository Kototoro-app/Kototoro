...
- 组内条目使用 `surfaceContainerLow`；Material 保持稳定不透明 Surface，iOS 使用主题色的半透明
  Surface 让页面背景适度透出。默认不画 Divider 或逐项描边，避免相邻亮线叠加。Divider 只表达
  账户、安全、危险操作等真实语义边界，不能与 `2dp` 结构间隙重复使用。
