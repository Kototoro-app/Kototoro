...
- 页面背景选项仅区分标准主题背景与动态图片背景；界面材质由所选界面风格自动决定。
- 普通深色保留主题生成的色调画布与完整 `surfaceContainer*` 层级，不等同于 AMOLED。
- AMOLED 的非图片画布为纯黑；容器必须使用单调递增的黑灰色阶，不能把所有 Surface 压成同一颜色。
- iOS Glass 在纯黑画布上仍绘制主题派生的内部 Tint；均匀黑色经过模糊后不会自行产生层级。
- 动态图片背景尊重用户选择，不由 AMOLED 强制替换；图片上的控件继续使用稳定 Surface 或 Glass Tint。
...
- 唯一例外是覆盖在封面图像上的极短标记：卡片进度环内的百分比标签、角徽标和阅读状态条允许
  使用 11 sp 的 `micro` 下限，但不得低于 11 sp；这些标记必须同时有图标、进度环或徽标容器
  作为语义载体，不能只靠文字。
...
- 分组标题位于组合 Surface 外，使用设置组标题语义排版。
- 组内每项拥有独立命中范围和状态反馈，但所有条目共同表达一个外轮廓：单项使用完整外圆角，
  首项只保留顶部外圆角，末项只保留底部外圆角，中间接缝使用小圆角。
- Material 3 Expressive 默认使用 `24dp` 外圆角、`4dp` 内圆角和 `2dp` 组内间隙；iOS Glass
  使用 `18dp` 外圆角、`4dp` 内圆角和同样的结构间隙，可增加一条低强调 Hairline。
- 组内条目在 Material 下使用 `surfaceContainer`，确保浅色、深色与 AMOLED 背景上都有稳定层级；
  iOS 在普通背景下同样使用不透明 `surfaceContainer`，仅在动态图片背景上使用半透明
  `surfaceContainerLow`。默认不画 Divider 或逐项描边，避免相邻亮线叠加。Divider 只表达账户、安全、
  危险操作等真实语义边界，不能与 `2dp` 结构间隙重复使用。
- 单行最小视觉高度为 `56dp`，双行最小视觉高度为 `68dp`；字体放大时允许自然增高。
- 标题使用条目主标题语义角色，说明使用 `bodyMedium` 对应的 supporting 角色，最多两行。
- 业务页面不得传递索引、首尾标记、圆角、Surface 色或组内间距；这些关系由共享设置模块推导。
- 组合 Surface 不是独立悬浮卡片堆叠：不使用逐项阴影，不扩大组内间距，不为普通设置项增加装饰边框。
