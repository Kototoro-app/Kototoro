...
| **`frameOverrunMs` P99** | -5.52 ms | **-6.22 ms** | 截止期安全余量扩大 0.70ms（P99 口径：≥99% 帧未超时；不据此断言零掉帧，见 [改进计划 §2](../architecture/reader-scene-improvement-plan-2026-09.md)） |
...
在应对 10,000 ~ 40,000 像素极高条漫长图时，不仅打破了 Android GPU 16384px 的硬件纹理上限天花板，同时斩获 **62% 的显存节省** 与 **50%+ 的 CPU 帧执行耗时压降**，在 120Hz 高刷物理设备上实现了无白屏的连续呈现（P99 口径下未见截止期超时；最慢约 1% 帧不在该口径内，见 [改进计划 §2](../architecture/reader-scene-improvement-plan-2026-09.md)）。
