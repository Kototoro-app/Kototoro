...
- [ ] 将 `CloudFlarePageState` 扩展为 `NORMAL`、`LOADING`、`MANAGED_CHALLENGE`、`INTERACTIVE_CHALLENGE`、`HARD_BLOCK`。
- [ ] 把 source、host、challenge URL、原始请求 URL、UA 和 headers 统一放入 CF 请求上下文。
- [ ] 人工 Activity 在关闭前执行真实 probe，而不是只依据 Cookie 变化。
- [ ] 增加主文档错误与 Turnstile 子资源错误的区分。
- [ ] 统一所有 parser、图片、下载、播放器和后台任务入口的 resolver 调用。
- [ ] 为页面状态、自动结果、cooldown、source 缺失和多 host 并发补充单元测试。
- [ ] 增加 `CaptchaResolver` 结构化日志，禁止记录完整 Cookie、token 和 Authorization。
