...
object PagedSpreadResolver {
    ...
    private fun layoutGroupInSlot(
        ...
        if (group.size == 1) {
            val spec = group[0]
            val fitted = fitPage(spec.geometryHint, viewportWidth, viewportHeight)
        ...
        val fittedFirst = fitPage(firstSpec.geometryHint, halfWidth, viewportHeight)
        val fittedSecond = fitPage(secondSpec.geometryHint, halfWidth, viewportHeight)
    ...
    private fun fitPage(hint: PageGeometryHint, containerW: Float, containerH: Float): PageSize {
        ...
        // Fit page aspect ratio inside container
        val scale = min(containerW / ratio, containerH)
        val fittedW = scale * ratio
        val fittedH = scale
        return PageSize(fittedW, fittedH)
    }
