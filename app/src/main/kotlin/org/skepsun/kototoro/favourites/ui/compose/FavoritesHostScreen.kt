...
fun KototoroFavoritesHostRoute(
    ...
    CompositionLocalProvider(LocalHazeState provides hazeState) {
        Box(modifier = Modifier.fillMaxSize()) {
            ...
            val duplicatesFinderState by viewModel.duplicatesFinderState.collectAsStateWithLifecycle()
            val duplicatesSummary by viewModel.duplicatesSummary.collectAsStateWithLifecycle()

            duplicatesFinderState?.let { finderState ->
                DuplicatesFinderDialog(
                    state = finderState,
                    onDismiss = { viewModel.dismissDuplicatesFinder() },
                    onCancel = { viewModel.cancelDuplicatesFinder() },
                    onQuickFix = { viewModel.startQuickDuplicatesFix() },
                    onFuzzyFix = { tolerance -> viewModel.startFuzzyDuplicatesFix(tolerance) },
                    onToggleGroupChecked = { index -> viewModel.toggleGroupChecked(index) },
                    onAcceptFuzzy = { viewModel.applyDuplicateDeletions() }
                )
            }

            duplicatesSummary?.let { summary ->
                DuplicatesSummaryDialog(
                    summary = summary,
                    onDismiss = { viewModel.dismissDuplicatesFinder() }
                )
            }
